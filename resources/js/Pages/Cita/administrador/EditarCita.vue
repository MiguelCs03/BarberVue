<template>
    <Head title="Editar Cita - Admin" />
  
    <AppLayout>
      <!-- Page Header -->
      <div class="mb-6">
        <Link
          :href="route('citas-admin.show', cita.id)"
        ...
        <p :style="{ color: 'var(--text-secondary)' }">
          Modifica la fecha, hora, barbero o estado de la cita
        </p>
      </div>
  ...
  const selectedEstado = ref((props.cita.estado || 'pendiente').toLowerCase());
  ...
  const hasChanges = computed(() => {
    return (
      ...
      selectedEstado.value !== originalValues.value.estado
    );
  });
  ...
  const cargarBarberosDisponibles = async () => {
    ...
    try {
      ...
      barberosDisponibles.value = response.data.barberos.map(barbero => ({
        ...barbero,
        avatar: generateAvatar(barbero.nombre || barbero.name),
        disponible: true
      }));
  ...
  const actualizarCita = () => {
    ...
    Swal.fire({
      ...
      text: 'Se actualizará la fecha, hora, barbero o estado de la cita',
      icon: 'question',
      ...
      if (result.isConfirmed) {
        router.put(route('citas-admin.update', props.cita.id), {
            fecha: selectedDate.value.date,
            hora: selectedHora.value,
            barbero_id: selectedBarbero.value,
            cliente_id: props.cita.cliente.id, // ← AGREGAR ESTA LÍNEA
            estado: selectedEstado.value.toLowerCase()
        });
      }
    });
  };
  ...
  onMounted(() => {
    ...
    selectedEstado.value = (props.cita.estado || 'pendiente').toLowerCase();
